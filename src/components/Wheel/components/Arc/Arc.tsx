...
export default PixiComponent('Arc', {
  ...
  applyProps: (instance:Graphics, oldProps:Props, newProps:Props)=>{
    const { x, y, outerRadius,innerRadius, startAngle,endAngle,fill,outerLine } = newProps;
    instance.clear();
    instance.beginFill(fill);
    instance.moveTo(x+(innerRadius * Math.cos(startAngle)),y+(innerRadius * Math.sin(startAngle)));
    instance.arc(x,y,outerRadius, startAngle, endAngle);
    instance.lineTo(x+(innerRadius * Math.cos(endAngle)),y+(innerRadius * Math.sin(endAngle)));
    instance.arc(x,y,innerRadius, endAngle, startAngle, true);
    instance.endFill();
    instance.lineStyle(outerLine.width,outerLine.color,0.5);
